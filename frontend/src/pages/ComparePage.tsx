...
import { useCompare, useSchools } from '../hooks/useSchools'
...
const METRICS: {
  ...
  { label: 'Ofsted', get: s => <OfstedBadge overall={s.ofsted_overall} date={s.ofsted_date} />, higher: null },
  { label: 'Attainment 8', get: s => <StatValue value={s.attainment_8} suppressed={s.ks4_suppressed} />, numVal: s => s.attainment_8, higher: 1 },
  { label: 'Progress 8', get: s => <StatValue value={s.progress_8} suppressed={s.ks4_suppressed} />, numVal: s => s.progress_8, higher: 1 },
  { label: 'Grade 5+ E&M', get: s => <StatValue value={s.pct_grade5_english_maths} suppressed={s.ks4_suppressed} suffix="%" />, numVal: s => s.pct_grade5_english_maths, higher: 1 },
  { label: 'Avg A-level pts', get: s => <StatValue value={s.avg_points_per_alevel_entry} suppressed={s.ks5_suppressed} />, numVal: s => s.avg_points_per_alevel_entry, higher: 1 },
  { label: '% Expected RWM', get: s => <StatValue value={s.pct_expected_rwm} suppressed={s.ks2_suppressed} suffix="%" />, numVal: s => s.pct_expected_rwm, higher: 1 },
  { label: 'FSM eligible %', get: s => fmt(s.pct_fsm6, '%'), numVal: s => s.pct_fsm6, higher: -1 },
          ...
          <table className="compare-table">
            <thead>
              <tr>
                ...
                {schools.map(s => (
                  <th key={s.urn} style={{ minWidth: 180 }}>
                    <div>
            ...
            <tbody>
              {METRICS.map(m => {
                ...
                const best = validVals.length > 0 && m.higher !== null
                ...
                return (
                  <tr key={m.label}>
                    <td><span className="metric-label">{m.label}</span></td>
                    {schools.map(s => {
